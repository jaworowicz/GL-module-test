<?php
define('IS_PUBLIC_PAGE', true);
require_once '../../includes/db.php';

// Pobierz SFID z URL
$sfid = $_GET['sfid'] ?? null;
if (!$sfid || !is_numeric($sfid)) {
    die('Nieprawidłowy kod lokalizacji');
}

// Sprawdź czy lokalizacja istnieje
$location_query = "SELECT name, address FROM sfid_locations WHERE id = ? AND is_active = 1";
$location_stmt = $pdo->prepare($location_query);
$location_stmt->execute([$sfid]);
$location = $location_stmt->fetch(PDO::FETCH_ASSOC);

if (!$location) {
    die('Lokalizacja nie istnieje lub została dezaktywowana');
}

// Sekcja obsługi dat
$period = $_GET['period'] ?? null;
$date_url_param = $_GET['date'] ?? date('Y-m-d');
$display_date = date('Y-m-d');
$date_from = date('Y-m-d');
$date_to = date('Y-m-d');
$show_kpi_section = ($period === 'today' || $period === 'current_month' || $period === null);

switch ($period) {
    case 'current_month':
        $date_from = date('Y-m-01');
        $date_to = date('Y-m-d');
        $display_date = date('Y-m-d');
        break;
    case 'previous_month':
        $prev_month_date = new DateTime('first day of last month');
        $date_from = $prev_month_date->format('Y-m-d');
        $date_to = $prev_month_date->format('Y-m-t');
        $display_date = $date_to;
        break;
    case 'yesterday':
        $yesterday_date = new DateTime('yesterday');
        $date_from = $yesterday_date->format('Y-m-d');
        $date_to = $yesterday_date->format('Y-m-d');
        $display_date = $date_from;
        break;
    case 'today': break;
    default:
        if (preg_match('/^\d{4}-\d{2}$/', $date_url_param)) {
             try {
                $month_date = new DateTime($date_url_param . '-01');
                $date_from = $month_date->format('Y-m-d');
                $date_to = $month_date->format('Y-m-t');
                $display_date = $date_to;
            } catch (Exception $e) { $display_date = date('Y-m-d'); }
        } elseif (preg_match('/^\d{4}-\d{2}-\d{2}$/', $date_url_param)) {
            $date_from = $date_url_param;
            $date_to = $date_url_param;
            $display_date = $date_url_param;
        }
        break;
}

$selected_date = $display_date;
$selected_date_obj = new DateTime($selected_date);
$kpi_year = $selected_date_obj->format('Y');
$kpi_month = $selected_date_obj->format('n');

$counters_query = "
    SELECT
        c.id, c.title, c.type, c.symbol, cat.name as category, cat.color as category_color,
        (SELECT SUM(dv.value) FROM licznik_daily_values dv WHERE dv.counter_id = c.id AND dv.date BETWEEN ? AND ?) as total_value
    FROM licznik_counters c
    LEFT JOIN licznik_categories cat ON c.category_id = cat.id
    WHERE c.sfid_id = ? AND c.is_active = 1
    GROUP BY c.id ORDER BY c.sort_order, c.title
";
$counters_stmt = $pdo->prepare($counters_query);
$counters_stmt->execute([$date_from, $date_to, $sfid]);
$counters = $counters_stmt->fetchAll(PDO::FETCH_ASSOC);

$trigger_redirect_to_yesterday = false;
$is_today_view = ($period === 'today' || $period === null);
if ($is_today_view && !empty($counters)) {
    $has_any_data = false;
    foreach ($counters as $counter) {
        if ($counter['total_value'] !== null && $counter['total_value'] > 0) {
            $has_any_data = true;
            break;
        }
    }
    if (!$has_any_data) {
        $trigger_redirect_to_yesterday = true;
    }
}

if ($show_kpi_section) {
    $has_kpi_query = "SELECT COUNT(*) FROM licznik_kpi_goals WHERE sfid_id = ? AND is_active = 1";
    $has_kpi_stmt = $pdo->prepare($has_kpi_query);
    $has_kpi_stmt->execute([$sfid]);
    $has_kpi_goals = $has_kpi_stmt->fetchColumn() > 0;

    $kpi_goals = [];
    if (!empty($counters) && $has_kpi_goals) {
        $kpi_query = "
            SELECT kg.id, kg.name, kg.total_goal, GROUP_CONCAT(klc.counter_id) as linked_counter_ids
            FROM licznik_kpi_goals kg
            LEFT JOIN licznik_kpi_linked_counters klc ON kg.id = klc.kpi_goal_id
            WHERE kg.sfid_id = ? AND kg.is_active = 1
            GROUP BY kg.id ORDER BY kg.id
        ";
        $kpi_stmt = $pdo->prepare($kpi_query);
        $kpi_stmt->execute([$sfid]);
        $kpi_goals = $kpi_stmt->fetchAll(PDO::FETCH_ASSOC);

        foreach ($kpi_goals as &$goal) {
            $linkedIds = $goal['linked_counter_ids'] ? explode(',', $goal['linked_counter_ids']) : [];
            $monthly_total = 0;
            if (!empty($linkedIds)) {
                $placeholders = str_repeat('?,', count($linkedIds) - 1) . '?';
                $realization_query = "
                    SELECT COALESCE(SUM(dv.value), 0) as total FROM licznik_daily_values dv
                    WHERE dv.counter_id IN ($placeholders) AND YEAR(dv.date) = ? AND MONTH(dv.date) = ?
                ";
                $params = array_merge($linkedIds, [$kpi_year, $kpi_month]);
                $realization_stmt = $pdo->prepare($realization_query);
                $realization_stmt->execute($params);
                $monthly_total = $realization_stmt->fetchColumn();
            }
            $goal['monthly_total'] = $monthly_total;
            $goal['progress_percent'] = $goal['total_goal'] > 0 ? min(100, ($monthly_total / $goal['total_goal']) * 100) : 0;
        }
    }
}
?>
<!DOCTYPE html>
<html lang="pl">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title><?= htmlspecialchars($location['name']) ?> - <?= date('d.m.Y', strtotime($selected_date)) ?></title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap" rel="stylesheet">
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet">
    <style>
        body { 
            font-family: 'Inter', sans-serif; 
            background: linear-gradient(-45deg, #0f2027, #203a43, #2c5364, #1c3d52); 
            background-size: 400% 400%; 
            animation: gradientBG 15s ease infinite;
            min-height: 100vh;
        }
        @keyframes gradientBG { 0% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } 100% { background-position: 0% 50%; } }
        .counter-card { background: rgba(30, 41, 59, 0.7); border: 1px solid rgb(51, 65, 85); border-radius: 1rem; transition: all 0.3s ease; }
        .counter-card:hover { background: rgba(30, 41, 59, 0.9); transform: translateY(-2px); box-shadow: 0 10px 25px rgba(0, 0, 0, 0.3); }
        .progress-bar { background: rgba(55, 65, 81, 0.5); border-radius: 0.25rem; overflow: hidden; border: 1px solid rgb(75, 85, 99); }
        .progress-fill { height: 100%; background: linear-gradient(90deg, rgb(34, 197, 94), rgb(16, 185, 129)); transition: width 0.3s ease; border-radius: 0.25rem; }
        .date-button { background: rgba(71, 85, 105, 0.6); border: 1px solid rgb(100, 116, 139); transition: all 0.2s ease; }
        .date-button:hover { background: rgba(100, 116, 139, 0.6); transform: scale(1.05); }
        .date-button.active { background: rgba(34, 197, 94, 0.8) !important; border-color: rgb(34, 197, 94) !important; transform: scale(1.05); box-shadow: 0 0 0 2px rgba(34, 197, 94, 0.3); }
        .date-button.today { background: rgba(34, 197, 94, 0.5); border-color: rgb(22, 163, 74); }
        .date-button.today:hover { background: rgba(34, 197, 94, 0.6); }
    </style>
</head>
<body class="bg-slate-900 text-white antialiased">
    <div class="container mx-auto px-4 py-8">
        <header class="text-center mb-8">
            <h1 class="text-4xl font-bold text-white mb-2"><?= htmlspecialchars($location['name']) ?></h1>
            <h2 class="text-2xl text-slate-300 mb-4">Kod lokalizacji: <?= htmlspecialchars($sfid) ?></h2>
            <div class="flex justify-center items-center gap-2 mb-4 flex-wrap">
                <button onclick="changeDate('yesterday')" class="date-button text-white px-4 py-2 rounded-lg" data-period="yesterday"><i class="fas fa-chevron-left mr-1"></i> Wczoraj</button>
                <button onclick="changeDate('current_month')" class="date-button text-white px-4 py-2 rounded-lg" data-period="current_month"><i class="fas fa-calendar-alt mr-1"></i> Ten miesiąc</button>
                <button onclick="changeDate('previous_month')" class="date-button text-white px-4 py-2 rounded-lg" data-period="previous_month"><i class="fas fa-calendar-alt mr-1"></i> Poprzedni miesiąc</button>
                <button onclick="changeDate('today')" class="date-button today text-white px-4 py-2 rounded-lg" data-period="today"><i class="fas fa-calendar-day mr-1"></i> Dziś</button>
            </div>
            <p class="text-slate-500 mt-2">Stan na dzień: <?= date('d.m.Y', strtotime($selected_date)) ?></p>
            
            <div class="flex items-center gap-4 rounded-lg bg-white p-6 shadow-md outline outline-black/5 dark:bg-gray-800 m-8">
              <span class="inline-flex shrink-0 rounded-full border border-pink-300 bg-pink-100 p-2 dark:border-pink-300/10 dark:bg-pink-400/10">
                <svg class="size-6 stroke-white dark:stroke-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                    <g>
                        <path fill="currentColor" d="M145.51,144.146c-0.007-0.007-0.017-0.007-0.024-0.014c-0.004,0-0.004,0-0.004,0L145.51,144.146z"/>
                        <path fill="currentColor" d="M162.102,143.25l-0.077,0.041h-0.004c-0.007,0.006-0.014,0.013-0.024,0.02L162.102,143.25z"/>
                        <path fill="currentColor" d="M347.32,303.504c-0.014,0.014-0.027,0.027-0.037,0.047l-0.148,0.202L347.32,303.504z"/>
                        <polygon fill="currentColor" points="325.794,185.453 325.797,185.453 325.8,185.447  "/>
                        <path fill="currentColor" d="M347.357,303.45l0.047-0.068l38.756-59.612l-0.094-0.094c1.301-2.628,1.945-5.48,1.945-8.304
                            c0.004-5.87-2.753-11.686-7.892-15.353l0.937,0.668l-0.934-0.668c-1.884-1.341-3.946-2.13-6.042-2.689v-1.476l-0.606,0.061
                            c-0.115-1.133-0.313-2.285-0.754-3.424c-0.637-1.651-1.739-3.282-3.357-4.496l-0.03-0.027l-32.434-23.406l0.014,0.007
                            c-1.405-1.018-3.064-1.591-4.852-1.611v-0.007h-0.051v0.007l-0.043,0.007v-0.014c-2.518,0.014-4.644,1.118-6.167,2.5
                            c-1.26,1.139-2.136,2.521-2.827,3.962l-9.038,14.626l-0.832,0.782c-11.393-8.135-18.304-12.206-38.042-22.875
                            c-28.276-15.285-42.26-26.864-49.347-34.634l-0.388-0.425l0.388,0.418c-1.584-1.732-5.186-5.77-9.806-10.972l-9.56-12.219
                            l-0.478,0.89c-7.666-8.667-16.128-18.251-23.248-26.332c-4.395-4.981-8.24-9.341-10.986-12.462
                            c-1.358-1.543-2.446-2.77-3.198-3.625l-0.408-0.465v-1.786h4.162c5.344,0,9.678-4.327,9.678-9.672V58.809
                            c0-4.906-3.667-8.91-8.398-9.543c9.28-7.865,14.935-20.185,15.036-34.837v-0.027v-0.021V14.22l-0.061-0.668l-0.016-0.175
                            l-0.037-0.175c-0.095-0.498-0.307-1.395-0.772-2.359l-0.128-0.269l-0.93-1.321l-0.165-0.229l-0.188-0.196
                            c-0.92-0.91-2.312-1.813-4.398-1.847h-0.01l-0.886,0.115l-0.064,0.013l-1.638,0.499l-0.778,0.236l-1.557,1.476l-0.218,0.209
                            l-0.182,0.243l-0.007,0.007l-1.166,1.017c-0.122-1.139-0.128-2.325-0.384-3.383L174.914,7.4c-0.246-0.984-0.556-1.988-1.034-2.986
                            l-0.02-0.041l-0.01-0.02c-0.286-0.586-0.923-1.496-1.779-2.352l-0.464-0.93l-3.04-0.91L168.043,0l-0.542,0.013
                            c-1.847,0.007-3.35,0.89-3.875,1.233l-0.057,0.041l-0.044,0.027c-1.261,0.863-2.127,1.765-2.959,2.554l-0.058,0.054l0.034-0.034
                            c-1.253,1.233-2.595,2.494-3.983,3.41c-1.399,0.924-2.767,1.489-4.189,1.584l-3.218,0.215V9.26
                            c-11.922,1.765-21.108,11.922-21.115,24.336c0,5.917,2.136,11.309,5.604,15.542c-5.328,0.013-9.641,4.326-9.652,9.664v11.862
                            c0.01,5.344,4.338,9.658,9.668,9.672h4.172v0.815c-2.71,3.357-4.269,7.461-4.269,11.707c0,2.905,0.684,5.83,2.059,8.519
                            l-0.054,0.054l1.493,2.339c0,0,0.32,0.499,0.772,1.206v26.036c0,2.359,0.334,4.562,1.156,6.584h-0.004
                            c0.004,0.006,0.007,0.006,0.01,0.013c0,0.007,0,0.014,0.004,0.02h0.003c1.281,3.168,3.754,5.399,6.487,6.504
                            c2.588,1.065,5.149,1.24,7.43,1.254h0.028c2.73-0.02,5.974-0.25,9.078-2.096c0.054-0.028,0.088-0.075,0.142-0.108
                            c5.968,9.375,12.333,19.383,18.514,29.094l-16.563,31.138l45.964,40.033l-8.832,153.724h11.666v3.012h-20.707v30.146h12.654v33.799
                            h-12.654v17.726h-17.837V512h197.653v-30.146h-17.83v-17.726h-12.657V430.33h12.657v-30.146h-20.714v-3.012h8.529h3.504
                            l-0.088-5.722l-1.324-84.508c1.126-0.983,2.224-2.062,3.184-3.39l0.041-0.06L347.357,303.45z M140.809,21.64
                            c3.07-3.06,7.282-4.947,11.956-4.947h0.077l0.084-0.007c4.819-0.371,8.408-2.952,10.955-5.21c1.277-1.132,2.325-2.21,3.104-2.925
                            l0.105-0.088l0.34,1.274c0.246,1.18,0.397,2.649,0.394,4.07c0.016,2.002-0.334,3.977-0.826,4.893l-0.492,0.97l0.485,0.984
                            l0.698,0.741l-0.425,2.38l2.534-1.308l0.35,0.088c1.301-0.027,2.322-0.472,3.42-0.99c1.088-0.54,2.21-1.22,3.343-1.942l0.037-0.02
                            c1.088-0.714,2.395-1.786,3.578-2.878c-0.846,14.517-7.599,25.172-17.621,30.416l-0.138,0.068l-0.121,0.094l-1.358,0.869
                            l-1.081,0.634v3.781h-14.561V48.99l-1.119-0.634c-5.172-2.898-8.674-8.411-8.674-14.76
                            C135.855,28.926,137.746,24.714,140.809,21.64z M133.665,72.518c-1.022,0-1.854-0.836-1.856-1.854V58.809
                            c0.002-1.017,0.835-1.86,1.856-1.86h9.799h2.184h14.561h2.184h9.796c1.031,0,1.857,0.836,1.861,1.86v11.855
                            c-0.004,1.025-0.83,1.854-1.861,1.854h-7.612h-4.367h-14.561h-2.184H133.665z M160.094,132.689
                            c-0.337,2.352-1.061,3.242-2.096,3.903c-1.072,0.666-2.861,0.983-5.072,0.977c-2.446,0.013-4.347-0.371-5.395-1.18
                            c-0.542-0.404-0.957-0.903-1.304-1.732c-0.34-0.829-0.58-2.008-0.58-3.646v-23.286c6.176,4.286,11.676,6.052,14.446,6.726V132.689z
                             M164.461,132.238v-21.479l-1.894-0.256l-0.054-0.007c-0.772-0.122-8.795-1.55-17.416-8.661c-0.923-1.448-1.445-2.271-1.445-2.271
                            l-0.044-0.067l-0.05-0.068c-1.476-1.982-2.177-4.274-2.18-6.572c0-3.012,1.226-5.985,3.562-8.128l0.708-0.647v-7.198h14.561v8.108
                            v6.935h4.367v-1.894c7.339,8.337,25.934,29.424,39.787,45.061l-17.338,32.592C179.427,155.745,171.477,143.257,164.461,132.238z
                             M346.373,408.002v14.51h-12.657v49.435h12.657v17.725h2.184h15.646v14.51H182.187v-14.51h17.832v-17.725h12.657v-49.435h-12.657
                            v-14.51h20.711V393.72h10.848H267.6h11.936h35.915h10.207v14.281H346.373z M340.954,298.962c-1.044,1.456-2.308,2.635-3.73,3.572
                            l-1.004,0.654l1.351,86.166h-7.545h-14.574h-35.915H267.6h-36.023h-15.215h-6.834l8.573-149.282l-44.168-38.469l15.781-29.674
                            l17.853-33.564l2.295,2.932l0.041,0.047l0.044,0.054c4.691,5.284,8.32,9.354,9.992,11.188c7.939,8.694,22.638,20.684,51.399,36.239
                            c21.79,11.802,27.14,15.023,40.751,24.903c2.47,3.478,7.242,11.794,7.235,24.276c0,2.925-0.263,6.079-0.876,9.462l4.293,0.788
                            c0.664-3.646,0.95-7.07,0.95-10.25c-0.007-11.397-3.626-19.632-6.484-24.391l12.421-20.104l0.074-0.188
                            c0.371-0.957,0.872-1.658,1.324-2.056l0.953-0.492l0.347,0.128l32.398,23.387c0.428,0.242,1.088,1.502,1.062,3.174
                            c0,0.93-0.179,1.941-0.542,2.878l-0.091,0.236c-2.636,0.836-5.018,2.157-7.043,3.855c-5.035,4.212-8.354,10.238-10.507,15.204
                            c-1.806,4.186-2.756,7.609-3.023,8.647l-8.559,13.904l3.717,2.284l8.916-14.483l0.081-0.344l0.067-0.263
                            c0.358-1.382,2.221-7.979,5.881-14.038c1.826-3.033,4.094-5.917,6.75-7.98c1.732-1.341,3.619-2.291,5.688-2.837l0.074-0.014
                            c0.654-0.115,1.321-0.175,1.988-0.175c2.217,0,4.441,0.66,6.392,2.056c3.013,2.15,4.614,5.533,4.617,8.984
                            c0,2.218-0.664,4.449-2.059,6.396l-0.027,0.04L340.954,298.962z"/>
                        <polygon fill="currentColor" points="177.283,19.389 176.95,19.605 176.92,19.625     "/>
                        <path fill="currentColor" d="M278.886,309.725c-5.416-7.582-10.201-15.076-13.698-21.351c-1.749-3.134-3.174-5.971-4.189-8.33
                            c-1.021-2.346-1.614-4.273-1.739-5.344l-4.334,0.546c0.243,1.84,0.964,3.963,2.059,6.524c3.842,8.903,12.462,22.82,22.581,36.272
                            c10.14,13.446,21.668,26.318,31.629,32.984l2.42-3.632C303.156,340.45,289.69,324.888,278.886,309.725z"/>
                        <path fill="currentColor" d="M229.02,458.137h19.326v-2.184v-25.394H229.02V458.137z M233.387,434.926h10.591v18.844h-10.591V434.926z"/>
                        <path fill="currentColor" d="M298.05,458.137h19.319v-2.184v-25.394H298.05V458.137z M302.417,434.926h10.584v18.844h-10.584V434.926z"/>
                        <path fill="currentColor" d="M263.537,458.137h19.319v-2.184v-25.394h-19.319V458.137z M267.901,434.926h10.591v18.844h-10.591V434.926z"/>
                        <path fill="currentColor" d="M216.993,95.055l20.883,15.191c-1.584,4.603-2.541,9.536-2.541,14.712
                            c0.007,25.429,20.606,46.032,46.028,46.032c25.422,0,46.021-20.603,46.028-46.032c0-5.176-0.957-10.109-2.541-14.712l20.883-15.191
                            c2.123-1.543,3.292-3.977,3.289-6.45c0-1.348-0.344-2.723-1.072-3.976l0.078,0.128l-0.07-0.122
                            c-1.466-2.534-4.132-3.982-6.885-3.976h-0.367l-0.361,0.074l-2.196,0.478L313.4,92.211c-0.391-0.377-0.786-0.748-1.186-1.112
                            l11.026-24.815l0.074-0.35l0.472-2.198v-0.364c0.003-2.588-1.318-5.062-3.565-6.565l0.02-0.027l-0.236-0.188l-0.236-0.128
                            c0,0-0.007-0.007-0.013-0.007l-0.738-0.431h-0.32c-0.926-0.364-1.884-0.607-2.854-0.607c-2.477,0-4.91,1.173-6.454,3.309
                            l-16.024,22.019c-0.408-0.115-0.818-0.216-1.23-0.317l-2.864-27.107c-0.418-4.036-3.822-7.137-7.909-7.137
                            c-4.088,0-7.492,3.101-7.909,7.137L270.59,80.43c-0.411,0.101-0.822,0.202-1.23,0.317l-16.024-22.019
                            c-1.543-2.136-3.976-3.309-6.453-3.309c-0.971,0-1.927,0.243-2.854,0.607h-0.32l-0.738,0.431c-0.007,0-0.01,0.007-0.01,0.007
                            l-0.24,0.128l-0.235,0.188l0.02,0.027c-2.248,1.503-3.569,3.977-3.565,6.565v0.364l0.471,2.198l0.074,0.35l11.026,24.815
                            c-0.401,0.364-0.795,0.734-1.186,1.112l-24.748-10.999l-2.197-0.478l-0.361-0.074h-0.367c-2.753-0.007-5.418,1.442-6.885,3.976
                            l-0.07,0.122l0.077-0.128c-0.728,1.253-1.071,2.628-1.071,3.976C213.701,91.078,214.87,93.512,216.993,95.055z M221.522,88.605
                            l0.013-0.061l0.007-0.006l0.111-0.061l0.05,0.013l0.095,0.048l29.334,13.028l1.058-1.254c1.89-2.23,4.061-4.239,6.436-6.005
                            l1.402-1.045l-13.254-29.829h-0.006l-0.01-0.061l0.064-0.115l0.007-0.007l0.054-0.014l0.111,0.054l19.319,26.547l1.56-0.586
                            c2.604-0.991,5.344-1.692,8.195-2.09l1.692-0.242l3.47-32.788l0.132-0.128l0.131,0.128l3.471,32.788l1.692,0.242
                            c2.85,0.398,5.59,1.098,8.195,2.09l1.56,0.586l19.319-26.547l0.112-0.054l0.054,0.014l0.006,0.007l0.064,0.115l-0.01,0.061h-0.007
                            l-13.254,29.829l1.402,1.045c2.376,1.766,4.546,3.774,6.436,6.005l1.058,1.254l29.334-13.028l0.095-0.048l0.05-0.013l0.111,0.061
                            l0.007,0.006l0.014,0.061l-0.054,0.108l0.068-0.048l-25.863,18.824l0.741,1.624c2.214,4.839,3.478,10.184,3.478,15.845
                            c0,10.561-4.273,20.098-11.191,27.026c-6.921,6.915-16.462,11.188-27.019,11.188c-10.558,0-20.098-4.272-27.02-11.188
                            c-6.918-6.928-11.191-16.465-11.191-27.026c0-5.661,1.264-11.006,3.478-15.845l0.742-1.624l-25.864-18.824l0.068,0.048
                            L221.522,88.605z"/>
                    </g>
                </svg>
              </span>
              <div>
                <p class="text-gray-700 dark:text-gray-400">
                  <span class="font-medium text-gray-950 dark:text-white">Funkcja eksperymentalna. Tylko widok "Dziś" działa stabilnie.</span>
                </p>
              </div>
            </div>
        </header>

        <section class="mb-12">
            <h3 class="text-2xl font-bold text-white mb-6">Liczniki</h3>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                <?php foreach ($counters as $counter): ?>
                <div class="counter-card p-6">
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            <h4 class="text-lg font-semibold text-slate-200"><?= htmlspecialchars($counter['title']) ?></h4>
                            <p class="text-slate-400 text-sm"><?= htmlspecialchars($counter['category'] ?? 'Bez kategorii') ?></p>
                        </div>
                    </div>
                    <div class="text-center h-12 flex items-center justify-center">
                        <?php if ($counter['total_value'] !== null): ?>
                            <div class="text-4xl font-bold text-green-400">
                                <?= $counter['total_value'] ?>
                                <?php if ($counter['type'] === 'currency' && $counter['symbol']): ?>
                                    <span class="text-2xl text-slate-400 ml-1"><?= htmlspecialchars($counter['symbol']) ?></span>
                                <?php endif; ?>
                            </div>
                        <?php else: ?>
                            <span class="text-lg text-slate-400 font-medium">Brak danych</span>
                        <?php endif; ?>
                    </div>
                </div>
                <?php endforeach; ?>
            </div>
        </section>

        <?php if ($show_kpi_section): ?>
        <section>
            <h3 class="text-2xl font-bold text-white mb-6">Cele KPI (<?= $selected_date_obj->format('F Y') ?>)</h3>
            <?php if (!$has_kpi_goals): ?>
                <div class="bg-slate-800/70 border border-slate-700 rounded-lg p-8 text-center">
                    <i class="fas fa-info-circle text-blue-400 text-3xl mb-4"></i>
                    <p class="text-slate-400 text-lg">Brak zdefiniowanych celów KPI dla tej lokalizacji.</p>
                </div>
            <?php elseif (empty($kpi_goals)): ?>
                 <div class="bg-slate-800/70 border border-slate-700 rounded-lg p-8 text-center">
                    <i class="fas fa-check-circle text-green-400 text-3xl mb-4"></i>
                    <p class="text-slate-400 text-lg">Wszystkie liczniki zostały uzupełnione.</p>
                </div>
            <?php else: ?>
                <div class="overflow-x-auto bg-slate-800/70 border border-slate-700 rounded-lg">
                    <table class="min-w-full text-sm text-left text-gray-300">
                        <thead class="text-xs text-gray-400 uppercase bg-gray-800">
                            <tr>
                                <th class="px-6 py-3">Cel</th>
                                <th class="px-6 py-3">Realizacja</th>
                                <th class="px-6 py-3">Cel miesięczny</th>
                                <th class="px-6 py-3 w-1/3">Postęp</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php foreach ($kpi_goals as $goal): ?>
                            <tr class="border-b border-slate-700 hover:bg-slate-800">
                                <td class="px-6 py-4 font-medium text-white"><?= htmlspecialchars($goal['name']) ?></td>
                                <td class="px-6 py-4 text-right font-mono"><?= $goal['monthly_total'] ?></td>
                                <td class="px-6 py-4 text-right font-mono"><?= $goal['total_goal'] ?></td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <div class="w-full progress-bar h-4 mr-3">
                                            <div class="progress-fill" style="width: <?= $goal['progress_percent'] ?>%"></div>
                                        </div>
                                        <div class="text-xs text-gray-400 w-10 text-right"><?= round($goal['progress_percent']) ?>%</div>
                                    </div>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                </div>
            <?php endif; ?>
        </section>
        <?php endif; ?>

        <footer class="text-center text-slate-500 mt-12">
            <p>Dane odświeżane w czasie rzeczywistym</p>
        </footer>
    </div>

    <script>
        function showNotification(message, type = 'info', duration = 5000) {
            const notification = document.createElement('div');
            notification.className = `fixed top-4 right-4 z-50 p-4 rounded-lg border max-w-md ${
                type === 'info' ? 'bg-blue-600/90 border-blue-500 text-white' : 
                type === 'warning' ? 'bg-yellow-600/90 border-yellow-500 text-white' : 
                'bg-gray-600/90 border-gray-500 text-white'
            }`;
            notification.innerHTML = `
                <div class="flex items-center">
                    <i class="fas fa-info-circle mr-2"></i>
                    <span>${message}</span>
                </div>
            `;
            document.body.appendChild(notification);
            setTimeout(() => {
                notification.remove();
            }, duration);
        }

        function updateActiveButton(period) {
            document.querySelectorAll('.date-button').forEach(btn => {
                btn.classList.remove('active');
                if (btn.getAttribute('data-period') !== 'today') {
                    btn.classList.remove('today');
                }
            });
            const activeButton = document.querySelector(`[data-period="${period}"]`);
            if (activeButton) {
                activeButton.classList.add('active');
                if (period !== 'today') {
                    const todayButton = document.querySelector(`[data-period="today"]`);
                    if (todayButton) todayButton.classList.remove('today');
                }
            }
        }
        function generateUrl(dateParam, period) {
            let sfid = new URLSearchParams(window.location.search).get('sfid') || '<?= htmlspecialchars($sfid) ?>';
            const baseUrl = window.location.pathname.split('?')[0];
            return `${baseUrl}?sfid=${sfid}&date=${dateParam}&period=${period}`;
        }
        function changeDate(period) {
            const today = new Date();
            let dateParam;
            switch(period) {
                case 'yesterday':
                    const yesterday = new Date(today);
                    yesterday.setDate(yesterday.getDate() - 1);
                    dateParam = yesterday.toISOString().split('T')[0];
                    break;
                case 'current_month':
                    dateParam = today.getFullYear() + '-' + String(today.getMonth() + 1).padStart(2, '0');
                    break;
                case 'previous_month':
                    let prevMonthDate = new Date(today.getFullYear(), today.getMonth() - 1, 1);
                    dateParam = prevMonthDate.getFullYear() + '-' + String(prevMonthDate.getMonth() + 1).padStart(2, '0');
                    break;
                case 'today':
                default:
                    dateParam = today.toISOString().split('T')[0];
                    break;
            }
            window.location.href = generateUrl(dateParam, period);
        }
        document.addEventListener('DOMContentLoaded', function() {
            const urlParams = new URLSearchParams(window.location.search);
            let activePeriod = urlParams.get('period');
            if (!activePeriod) {
                const dateParam = urlParams.get('date');
                const today = new Date().toISOString().split('T')[0];
                activePeriod = (!dateParam || dateParam === today) ? 'today' : 'today';
            }
            if (activePeriod) {
                updateActiveButton(activePeriod);
            }
        });
    </script>
    
    <?php if ($trigger_redirect_to_yesterday): ?>
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            showNotification('Brak danych na dziś, przełączam widok na wczorajszy dzień.', 'warning', 10000);
            
            setTimeout(function() {
                changeDate('yesterday');
            }, 10000);
        });
    </script>
    <?php endif; ?>
    <?php if ($is_today_view): ?>
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // 1. Sprawdź po załadowaniu strony, czy została ona automatycznie odświeżona
        if (sessionStorage.getItem('wasAutoRefreshed') === 'true') {
            // Jeśli tak, pokaż komunikat "Odświeżono"
            showNotification('Odświeżono', 'info', 4000); // Komunikat na 4 sekundy

            // Wyczyść flagę, aby komunikat nie pokazał się przy ręcznym odświeżeniu
            sessionStorage.removeItem('wasAutoRefreshed');
        }

        // 2. Ustaw interwał, który będzie odświeżał stronę co 60 sekund
        setInterval(function() {
            // Ustaw flagę w pamięci sesji przeglądarki, aby poinformować następne załadowanie strony
            sessionStorage.setItem('wasAutoRefreshed', 'true');
            
            // Przeładuj stronę
            window.location.reload();
        }, 60000); // 60000 ms = 60 sekund
    });
</script>
<?php endif; ?>
</body>
</html>